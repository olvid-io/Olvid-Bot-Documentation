...
- Sans [chiffrement TLS](#chiffrement-tls), les clés clientes ne servent qu'à compartimenter les appels API, et ne peuvent être utilisées à des fins d'authentification.
:::

## Creer un bot keycloak
:::{note}
Cette section ne s'adresse qu'aux clients qui ont un plugin Keycloak deploye 😬. 

Si vous ne savez pas ce que c'est vous pouvez cliquer 👉️ [ici](https://www.olvid.io/faq/olvid-management-console) 👈️ pour en apprendre plus sur cette fonctionalite premium.
:::
...
% todo: remplacer par un lien vers un référentiel GitHub ou un exemple de répertoire ?
:::
...
```shell
# générez la clé privée serveur
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out server.key
# générez la demande de certificat serveur
...
```shell
# générez la clé privée client
openssl genpkey -algorithm RSA -pkeyopt rsa_keygen_bits:2048 -out client.key

# générez la demande de certificat client (vous pouvez laisser les valeurs par défaut)
openssl req -new -key client.key -out client.csr
# définissez le nom commun à ce que vous voulez pour identifier le client
...
Chaque sous-répertoire (0001, 0002) doit contenir son propre fichier `backup_seed.txt`. Le daemon incrémentera et créera un nouveau répertoire.
...
% todo:: ajouter quelque chose sur l'API de stockage
