...
### CLI Options
Il existe deux façons de lancer le daemon avec des options en ligne de commande :

Pour définir des options CLI persistantes, mettez à jour votre `docker-compose.yaml` pour ajouter l'attribut `command` à votre service daemon :
```yaml
services:
  daemon:
    image: olvid/bot-daemon
...
Pour utiliser des options temporaires, utilisez la commande `docker compose run`. Par exemple, exécutez :
```shell
docker compose run --rm daemon --list
```

Cela affichera les clés client disponibles et se terminera.
