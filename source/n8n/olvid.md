...
:::{note}
Dans la section suivante vous allez déployer un daemon Olvid et une instance N8N à l'aide de docker et docker-compose.

Si vous souhaitez utiliser une instance de daemon et/ou de N8N déjà déployée, vous pouvez ne déployer qu'une partie des services suivants.
Il vous faudra seulement remplir correctement les champs `Client Key` et `Daemon Url` au moment de la création des credentials dans N8N.
:::
...
[//]: # (TODO test without N8N_REINSTALL_MISSING_PACKAGES)
...
3. **Rendez-vous** sur la page de paramètres
   ```{figure} /_static/images/n8n-settings.png
...
4. **Cliquer** sur `Community Nodes` puis **cliquer** sur `Install a community node`.
   ...
   ```{figure} /_static/images/n8n-community-nodes-page.png
...
5. **Entrer** `n8n-nodes-olvid` à la place de _npm Package Name_, prendre le temps de considérer les risques d'installer un noeud communautaire avant de **cocher** la case et **cliquer** sur `Install`.
   ...
   ```{figure} /_static/images/n8n-install-olvid-node.png
...
```{figure} /_static/images/n8n-workflow-olvid-page.png
...
2. Vous pouvez **choisir** indépendamment une action ou un trigger.
   ...
   ```{figure} /_static/images/n8n-select-trigger.png
...
3. **Ouvrez** votre noeud et créez les credentials en **cliquant** sur `Select Credential`.
   ...
   ```{figure} /_static/images/n8n-olvid-credentials.png
...
6. **Saisissez** ensuite la valeur du champ `Daemon Url` pour mettre `http://daemon:50051` (si n8n a été créé dans le même fichier docker-compose.yaml, sinon saisir l'URL qui permet d'atteindre votre daemon).
